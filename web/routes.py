...
async def voice_refine(req: VoiceRefineRequest, request: Request,
                       user_id: str = Depends(get_current_user)):
    """语音输入完成后，对识别文本进行润色修复，保障语义顺畅"""
    agent = request.app.state.get_agent(user_id)
    result = await asyncio.to_thread(agent.refine_voice_text, req.text)
    return JSONResponse({"refined": result})
